...
app.post("/add-product", (req, res) => {
  ...
  readFile(PRODUCTS_FILE, (err, products) => {
    ...
    fs.writeFile(PRODUCTS_FILE, JSON.stringify(products, null, 2), (err) => {
      if (err) return res.status(500).json({ error: "Error writing products file" });
...
app.delete("/products/:id", (req, res) => {
  readFile(PRODUCTS_FILE, (err, products) => {
    ...
    if (updatedProducts.length === products.length) return res.status(404).json({ error: "Product not found" });
    fs.writeFile(PRODUCTS_FILE, JSON.stringify(updatedProducts, null, 2), (err) => {
      if (err) return res.status(500).json({ error: "Error updating products file" });
...
app.get("/projects", (req, res) => {
  readFile(PROJECTS_FILE, (err, projects) => {
    if (err) return res.status(500).json({ error: "Error reading projects file" });
    res.json(projects);
  });
});
...
app.post("/add-project", (req, res) => {
  ...
  readFile(PROJECTS_FILE, (err, projects) => {
    ...
    fs.writeFile(PROJECTS_FILE, JSON.stringify(projects, null, 2), (err) => {
      if (err) return res.status(500).json({ error: "Error writing projects file" });
...
app.delete("/projects/:id", (req, res) => {
  readFile(PROJECTS_FILE, (err, projects) => {
    ...
    if (updatedProjects.length === projects.length) return res.status(404).json({ error: "Project not found" });
    fs.writeFile(PROJECTS_FILE, JSON.stringify(updatedProjects, null, 2), (err) => {
      if (err) return res.status(500).json({ error: "Error updating projects file" });
